...
export default {
  ...
  initialize(container) {
    ...
    function showMarkerJs(img, config) {
      let mark = new markerjs2.MarkerArea(img, { previousState: config });
    ...
    function showCropro(img, config) {
      // create an instance of CropArea and pass the target image reference as a parameter
      let cropArea = new cropro.CropArea(img, { previousState: config });
    ...
    document.addEventListener('click', function(e) {
      if (e.target.tagName === "IMG" && hasParentWithClass(e.target, "d-editor-preview")){
        ...
        showMarkerJs(e.target); 
      }
    });

    ["composer:opened", "composer:replace-text"].forEach((event) => {
      appEvents.on(event, () => later(addListenersToImages, 10000))
    });

    function addListenersToImages() {
      let images = document.querySelectorAll(".d-editor-preview img");
      for (let i = 0; i < images.length; i++) {
        images[i].onmouseover = () => { addElement(images[i]) };
        images[i].onmouseout = (e) => { hideElement(images[i], e) };
      }
    };
    ...
    function addElement(image) {
      if (!btnContainer) {
        ...
        annotateBtn.addEventListener('click', function(e) {        
          showMarkerJs(lastHoverImage); 
        });

        cropBtn.addEventListener('click', function(e) {        
          showCropro(lastHoverImage); 
        });
      }
    ...
    function hideElement(image, event) {
      if (event.toElement && event.toElement.classList.contains("image-annotator-hovering-button")) {
        return;
      }
      if (btnContainer) {
